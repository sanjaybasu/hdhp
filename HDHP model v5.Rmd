...
svyquantile(~prmshdpa, covwt.design,na.rm=T, quantiles = c(.025))[1]
svyquantile(~prmshdpa, covwt.design,na.rm=T, quantiles = c(.975))[1]

# Change in premium: HDP => PPO
svymean(~prmshdpa, covwt.design,na.rm=T)[1]  + (1/.85)*(-1.23 * svymean(~g43ann, covwt.design,na.rm=T)[1]-0.55 * (svymean(~ddctsppo, covwt.design,na.rm=T)[1]-svymean(~ddctshdp, covwt.design,na.rm=T)[1])-82.13*(svymean(~e6b, covwt.design,na.rm=T)[1]-svymean(~g6b, covwt.design,na.rm=T)[1])/100)

  svyquantile(~prmshdpa, covwt.design,na.rm=T, quantiles = c(.025))[1]  + (1/.85)*(-1.23 * svyquantile(~g43ann, covwt.design,na.rm=T, quantiles = c(.025))[1]-0.55 * (svyquantile(~ddctsppo, covwt.design,na.rm=T, quantiles = c(.025))[1]-svyquantile(~ddctshdp, covwt.design,na.rm=T, quantiles = c(.025))[1])-82.13*(svyquantile(~e6b, covwt.design,na.rm=T, quantiles = c(.025))[1]-svyquantile(~g6b, covwt.design,na.rm=T, quantiles = c(.025))[1])/100)

svyquantile(~prmshdpa, covwt.design,na.rm=T, quantiles = c(.975))[1]  + (1/.85)*(-1.23 * svyquantile(~g43ann, covwt.design,na.rm=T, quantiles = c(.975))[1]-0.55 * (svyquantile(~ddctsppo, covwt.design,na.rm=T, quantiles = c(.975))[1]-svyquantile(~ddctshdp, covwt.design,na.rm=T, quantiles = c(.975))[1])-82.13*(svyquantile(~e6b, covwt.design,na.rm=T, quantiles = c(.975))[1]-svyquantile(~g6b, covwt.design,na.rm=T, quantiles = c(.975))[1])/100)

# Annual HSA Firm Contribution: Single - HDP
svymean(~g43ann, covwt.design,na.rm=T)[1]
svyquantile(~g43ann, covwt.design,na.rm=T, quantiles = c(.025))[1]
svyquantile(~g43ann, covwt.design,na.rm=T, quantiles = c(.975))[1]

# Annual Contribution for Single coverage - HDP
svymean(~wkrshdpa, covwt.design,na.rm=T)[1]
svyquantile(~wkrshdpa, covwt.design,na.rm=T, quantiles = c(.025))[1]
svyquantile(~wkrshdpa, covwt.design,na.rm=T, quantiles = c(.975))[1]
...
svyquantile(~ddctshdp, covwt.design,na.rm=T, quantiles = c(.025))[1]
svyquantile(~ddctshdp, covwt.design,na.rm=T, quantiles = c(.975))[1]
...
svyquantile(~ddctsppo, covwt.design,na.rm=T, quantiles = c(.025))[1]
svyquantile(~ddctsppo, covwt.design,na.rm=T, quantiles = c(.975))[1]


# HDHP net cost
empcost_hdhp = df$prmshdpa -  df$g43ann - df$wkrshdpa
empcost_ppo = df$prmshdpa + (1/.85)*(-1.23*df$g43ann-0.55*(df$ddctsppo-df$ddctshdp)-82.13*(df$e6b-df$g6b)/100) - df$wkrsppoa

summary(empcost_hdhp)
summary(empcost_ppo)

quantile(empcost_hdhp,c(.025,.975),na.rm=T)
quantile(empcost_ppo,c(.025,.975),na.rm=T)

summary(empcost_ppo-empcost_hdhp)
quantile(empcost_ppo-empcost_hdhp,c(.025,.975),na.rm=T)
...
# ```{r mem cost}
# 
# spark_install(version = "2.4.0", hadoop_version = "2.6")
# sc <- spark_connect(master = "local", version = "2.4.0")
# sparkR.session()
# df <- read.orc("dbfs:/mnt/staging/presto-looker-s3-prod/analytics_claims_medical/analytics_claims_medical.orc")
# df_sub <- sample(df_sub, withReplacement = F, fraction = 0.01)
# df_sub <- as.data.frame(df_sub)
# 
# df_sub2 <- df_sub %>%
#   dplyr::filter(lubridate::year(service_date_start)==2018) %>%
#   dplyr::filter(str_detect(plan_name, "HDHP") | str_detect(plan_name,"CDHP") | str_detect(plan_name,"HSA") | str_detect(plan_name,"HRA") | str_detect(plan_name,"PPO") |    str_detect(plan_name,"EPO")) %>%
#   dplyr::mutate((hdhp = str_detect(plan_name,"HDHP") | str_detect(plan_name,"CDHP") | str_detect(plan_name,"HSA") | str_detect(plan_name,"HRA")),
#          replace_na(hdhp,0)) %>%
#   dplyr::select(person_id,deductible_amount,copay_amount,coinsurance_amount,hdhp) %>%
#   dplyr::group_by(person_id,hdhp) %>%
#   dplyr::summarise(deductible_amount = sum(deductible_amount),
#            copay_amount = sum(copay_amount),
#            coinsurance_amount = sum(coinsurance_amount))
# 
# summary(df_sub2$deductible_amount[df_sub2$deductible_amount>0 & df_sub2$hdhp==1])
# summary(df_sub2$copay_amount[df_sub2$copay_amount>0 & df_sub2$hdhp==1])
# summary(df_sub2$coinsurance_amount[df_sub2$coinsurance_amount>0 & df_sub2$hdhp==1])
# 
# quantile(df_sub2$deductible_amount[df_sub2$deductible_amount>0 & df_sub2$hdhp==1],c(.025,.975))
# quantile(df_sub2$copay_amount[df_sub2$copay_amount>0 & df_sub2$hdhp==1],c(.025,.975))
# quantile(df_sub2$coinsurance_amount[df_sub2$coinsurance_amount>0 & df_sub2$hdhp==1],c(.025,.975))
# 
# summary(df_sub2$deductible_amount[df_sub2$deductible_amount>0 & df_sub2$hdhp==0])
# summary(df_sub2$copay_amount[df_sub2$copay_amount>0 & df_sub2$hdhp==0])
# summary(df_sub2$coinsurance_amount[df_sub2$coinsurance_amount>0 & df_sub2$hdhp==0])
# 
# quantile(df_sub2$deductible_amount[df_sub2$deductible_amount>0 & df_sub2$hdhp==0],c(.025,.975))
# quantile(df_sub2$copay_amount[df_sub2$copay_amount>0 & df_sub2$hdhp==0],c(.025,.975))
# quantile(df_sub2$coinsurance_amount[df_sub2$coinsurance_amount>0 & df_sub2$hdhp==0],c(.025,.975))
# ```
...
# icer
mean(mean(na.omit(empcost_ppo-empcost_hdhp))+emp_cost_tot)/mean(mem_qaly_tot)
quantile(((quantile(na.omit(empcost_ppo-empcost_hdhp),c(.025))+emp_cost_tot)/(mem_qaly_tot)),c(.975))
quantile(((quantile(na.omit(empcost_ppo-empcost_hdhp),c(.975))+emp_cost_tot)/(mem_qaly_tot)),c(.025))


mean(mean(na.omit(empcost_ppo-empcost_hdhp))+emp_cost_tot + mem_cost_tot + mcare_cost_tot)/mean(mem_qaly_tot)
quantile(((quantile(na.omit(empcost_ppo-empcost_hdhp),c(.025))+emp_cost_tot + mem_cost_tot + mcare_cost_tot)/(mem_qaly_tot)),c(.975))
quantile(((quantile(na.omit(empcost_ppo-empcost_hdhp),c(.975))+emp_cost_tot + mem_cost_tot + mcare_cost_tot)/(mem_qaly_tot)),c(.025))
...
# Fig on change in util/cost

calculated from Lo Sasso table 4 column 2 [OLS] how much change in total spending would occur without HSA and with lower deductible including in-network coinsurance, and hosp/surgery deductible (significant variables at 5% alpha), with an 85% loss ratio from claims to premium
DOI: 10.1111/j.1539-6975.2009.01346.x

```{r fig}

(1/.85)*(-1.23 * svymean(~g43ann, covwt.design,na.rm=T)[1])

(1/.85)*(-0.55 * (svymean(~ddctsppo, covwt.design,na.rm=T)[1]-svymean(~ddctshdp, covwt.design,na.rm=T)[1]))
         
(1/.85)*(-82.13*(svymean(~e6b, covwt.design,na.rm=T)[1]-svymean(~g6b, covwt.design,na.rm=T)[1])/100)




(1/.85)*(-1.23 * svyquantile(~g43ann, covwt.design,na.rm=T, quantiles = c(.025))[1])

(1/.85)*(-0.55 * (svyquantile(~ddctsppo, covwt.design,na.rm=T, quantiles = c(.025))[1]-svyquantile(~ddctshdp, covwt.design,na.rm=T, quantiles = c(.025))[1]))
         
(1/.85)*(-82.13*(svyquantile(~e6b, covwt.design,na.rm=T, quantiles = c(.025))[1]-svyquantile(~g6b, covwt.design,na.rm=T, quantiles = c(.025))[1])/100)




(1/.85)*(-1.23 * svyquantile(~g43ann, covwt.design,na.rm=T, quantiles = c(.975))[1])

(1/.85)*(-0.55 * (svyquantile(~ddctsppo, covwt.design,na.rm=T, quantiles = c(.025))[1]-svyquantile(~ddctshdp, covwt.design,na.rm=T, quantiles = c(.975))[1]))
         
(1/.85)*(-82.13*(svyquantile(~e6b, covwt.design,na.rm=T, quantiles = c(.025))[1]-svyquantile(~g6b, covwt.design,na.rm=T, quantiles = c(.975))[1])/100)
